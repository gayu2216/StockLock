...
# Initialize Gemini API - UPDATED FOR CURRENT API VERSION
def initialize_gemini():
    try:
        # Hard-coded API key (in production, you would use environment variables or secrets management)
        GEMINI_API_KEY = "AIzaSyA2EP7PqgtNGbbt96OLMbTolNAplzKjK7k"
        genai.configure(api_key=GEMINI_API_KEY)
        
        # Updated model name to use the latest available model
        # If 'gemini-pro' doesn't work, you may need to use a different model name
...
# Function to get AI analysis of investment
def get_investment_analysis(ticker, results, params):
    model = initialize_gemini()
    
    if not model:
        return "AI analysis unavailable: Could not initialize Gemini API."
    ...
    Provide concise investment advice and highlight potential hidden risks in no more than 5 lines total.
    Format your response in two parts:
    1. Investment suggestion (1-2 sentences)
    2. Hidden risks (1-2 sentences)
    """
    ...
    try:
        ...
        # Provide a fallback analysis when AI is unavailable
...
def generate_fallback_analysis(ticker, results, params):
    ...
    if roi > 100:
        suggestion = f"Based on historical performance, {ticker} has shown strong returns and may be a good candidate for DCA investing."
    elif roi > 50:
        suggestion = f"{ticker} has demonstrated decent historical performance, making it potentially suitable for your DCA strategy."
    else:
        suggestion = f"Consider {ticker} as part of a diversified portfolio, as its historical returns are moderate."
    
    risks = f"Be aware that past performance doesn't guarantee future results, and {ticker} may face industry-specific challenges or market volatility."
    
    return f"**Investment suggestion:** {suggestion}\n\n**Hidden risks:** {risks}"
...
def main():
    ...
    if st.session_state.stock_data and st.session_state.annual_returns:
        ...
        if 'results' in st.session_state and st.session_state.results:
            ...
            # MOVED: AI Analysis to the end of the page and left-aligned in its own container outside columns
            ...
            # Display AI analysis in a colored box
            st.markdown(f"""
